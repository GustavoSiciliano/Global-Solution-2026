...
function mapHistorico(h: any): HistoricoAlerta {
  return {
    id_historico: h.idHistorico ?? h.id_historico,
    id_alerta: h.idAlerta ?? h.id_alerta,
    id_usuario: h.idUsuario ?? h.id_usuario,
    ds_acao: h.dsAcao ?? h.ds_acao ?? "VISUALIZADO",
    ds_observacao: h.dsObservacao ?? h.ds_observacao ?? "",
    dt_acao: h.dtAcao ?? h.dt_acao ?? "",
  };
}
// ── HTTP helpers ─────────────────────────────────────────────────────────────
...
export const javaApi = {
  ...
  atualizarRegiao: async (
    id: number,
    r: { nmRegiao: string; dsEstado: string; dsBioma: string },
  ): Promise<Regiao> => mapRegiao(await javaPut(`/regioes/${id}`, r)),
  ...
  getAlertas: async (): Promise<AlertaDetalhado[]> => {
    try {
      ...
      return ((alertRaw as any[]) ?? []).map((a: any): AlertaDetalhado => {
        ...
        const historico: HistoricoAlerta[] = (a.historico ?? []).map(
          mapHistorico,
        );
  ...
  // GET /regioes/situacao/cep/{cep} → RegiaoComPrevisao com leitura e previsão
  buscarPorCep: async (cep: string): Promise<RegiaoComPrevisao | null> => {
    const cepNum = cep.replace(/\D/g, "");
    if (cepNum.length !== 8) return null;
    try {
      const raw = await javaGet(`/regioes/situacao/cep/${cepNum}`);
      if (!raw?.regiao) return null;
      const regiao = mapRegiao(raw.regiao);
      const leitura = raw.ultimaLeitura
        ? mapLeitura(raw.ultimaLeitura)
        : mockRegioesComPrevisao[0].leitura;
      const previsao = raw.ultimaPrevisao
        ? mapPrevisao(raw.ultimaPrevisao)
        : prevPadrao;
      const fontes = await javaGet("/fontes")
        .then((f: any[]) => f.map(mapFonte))
        .catch(() => mockFontes);
      const fonte =
        fontes.find((f: FonteSatelital) => f.id_fonte === leitura.id_fonte) ??
        fontePadrao;
      return { ...regiao, leitura, previsao, fonte };
    } catch {
      return null;
    }
  },
  ...
  // POST /alertas/historico → TB_HISTORICO_ALERTA
  registrarHistorico: async (
    idAlerta: number,
    idUsuario: number,
    dsAcao: "VISUALIZADO" | "RECONHECIDO" | "RESOLVIDO" | "IGNORADO",
    dsObservacao?: string,
  ): Promise<void> => {
    try {
      await javaPost("/alertas/historico", {
        idAlerta,
        idUsuario,
        dsAcao,
      ...
      /* silencioso — historico nao bloqueia fluxo principal */
    }
  },
