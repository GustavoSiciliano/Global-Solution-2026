...
const integrantes = [
  { nome: "Gustavo Siciliano", rm: "RM568419" },
  { nome: "Gustavo J. Silva", rm: "RM567926" },
  { nome: "Samuel K. Lima", rm: "RM567614" },
];
      ...
      <div className="max-w-6xl mx-auto px-5 py-10">
        <div className="flex flex-col md:flex-row gap-8 justify-between mb-8">
          <div>
            ...
            <div className="mt-4 space-y-1.5">
              {integrantes.map((i) => (
                <div key={i.rm} className="flex items-center gap-2">
                  <span className="font-mono text-[10px] font-semibold text-accent bg-accent-bg border border-accent/20 px-1.5 py-0.5 rounded">
                    {i.rm}
                  </span>
                  <span className="text-xs text-dim">{i.nome}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between pt-6 border-t border-border gap-3">
          <span className="text-xs text-dim">
            © 2026 AgroSat · FIAP · Global Solution 2026/1
          </span>
          <span className="text-xs text-dim">1TDS Agosto · FIAP 2026</span>
        </div>
      </div>
    </footer>
  );
