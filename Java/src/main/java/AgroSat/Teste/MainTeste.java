...
public class MainTeste {
    ...
    static void menuRegioes() {
        ...
        while (loop) {
            ...
            System.out.println("  4. Cadastrar por cidade e estado");
            System.out.println("  5. Consultar dados por CEP");
            ...
            switch (scanner.nextLine().trim()) {
                ...
                case "3" -> {
                    System.out.print("> Sigla do estado (ex: SP, MT, AM): ");
                    exibir(get("/regioes/estado/" + scanner.nextLine().trim().toUpperCase()));
                }
                case "4" -> {
                    // Coordenadas e bioma buscados automaticamente
                    System.out.print("> Cidade (ex: Cuiaba): ");
                    String cidade = scanner.nextLine().trim();
                    System.out.print("> Estado (ex: MT): ");
                    String estado = scanner.nextLine().trim().toUpperCase();
                    aguardar("Buscando coordenadas e bioma...");
                    exibir(post("/regioes/buscar-por-cidade",
                            json("nmCidade", cidade, "dsEstado", estado)));
                }
                case "5" -> {
                    // Retorna regiao + ultima leitura + ultima previsao + alertas pendentes
                    System.out.print("> CEP (ex: 78000000): ");
                    String cep = scanner.nextLine().trim().replaceAll("[^0-9]", "");
                    aguardar("Consultando dados da regiao...");
                    exibir(get("/regioes/situacao/cep/" + cep));
                }
                case "6" -> {
                    System.out.print("> ID da regiao: ");
                    String id = scanner.nextLine().trim();
                    String atual = get("/regioes/" + id);
                    if (atual.contains("\"erro\"")) {
                        exibir(atual);
                        break;
                    }
                    System.out.println("  (Enter para manter o valor atual)");
                    String nm = lerOuManter(extrairCampo(atual, "nmRegiao"), "Nome");
                    String est = lerOuManterUpper(extrairCampo(atual, "dsEstado"), "Estado");
                    String bio = lerOuManter(extrairCampo(atual, "dsBioma"), "Bioma");
                    // Lat/lon sao mantidos do registro atual — usuario nao precisa informar
                    String lat = extrairCampo(atual, "nrLatitude");
                    String lon = extrairCampo(atual, "nrLongitude");
        ...
        while (loop) {
            ...
            switch (scanner.nextLine().trim()) {
                ...
                case "4" -> {
                    ...
                    String id = scanner.nextLine().trim();
                    String atual = get("/usuarios/" + id);
                    if (atual.contains("\"erro\"")) {
                        exibir(atual);
                        break;
                    }
                    // Mostra valor atual em [colchetes] — Enter mantem, qualquer texto substitui
                    System.out.println("  (Enter para manter o valor atual)");
                    String nm = lerOuManter(extrairCampo(atual, "nmUsuario"), "Nome");
                    String email = lerOuManter(extrairCampo(atual, "dsEmail"), "Email");
                    String perf = lerOuManterUpper(extrairCampo(atual, "dsPerfil"), "Perfil (ADMIN/PRODUTOR/GESTOR)");
    ...
    static void menuLeituras() {
        ...
        while (loop) {
            ...
            System.out.println("  4. Registrar nova leitura");
            System.out.println("  5. Deletar");
            System.out.println("  0. Voltar");
            System.out.print("\n> Escolha: ");
            switch (scanner.nextLine().trim()) {
                ...
                case "3" -> {
                    // ID numerico da regiao — use opcao 1 de Regioes para consultar
                    System.out.print("> ID da regiao (numero): ");
                    exibir(get("/leituras/regiao/" + scanner.nextLine().trim()));
                }
                case "4" -> {
                    // Cria uma nova leitura para a regiao informada
                    // Temperatura, precipitacao, umidade e NDVI sao coletados automaticamente
                    System.out.print("> ID da regiao (numero): ");
                    String id = scanner.nextLine().trim();
    ...
    static void menuPrevisoes() {
        ...
        while (loop) {
            ...
            switch (scanner.nextLine().trim()) {
                ...
                case "2" -> {
                    System.out.print("> ID da previsao: ");
    ...
    static void menuAlertas() {
        ...
        while (loop) {
            ...
            System.out.println("  3. Listar por regiao");
            System.out.println("  4. Detalhar alerta");
            System.out.println("  5. Historico de acoes de um alerta");
            ...
            switch (scanner.nextLine().trim()) {
                ...
                case "2" -> {
                    exibir(get("/alertas/pendentes"));
                    linha("  >> Alertas com flResolvido = N ainda nao foram tratados.");
                }
                case "3" -> {
                    System.out.print("> ID da regiao (numero): ");
                ...
                case "4" -> {
                    ...
                    String resp = get("/alertas/" + idA);
                    exibir(resp);
                    if (!resp.contains("\"erro\"") && resp.contains("flResolvido")) {
                        boolean resolvido = resp.contains("\"flResolvido\": \"S\"") || resp.contains("\"flResolvido\":\"S\"");
                        linha("  >> Status: " + (resolvido ? "RESOLVIDO" : "PENDENTE"));
                    }
                }
                case "5" -> {
                    System.out.print("> ID do alerta: ");
                    String idA = scanner.nextLine().trim();
                    String hist = get("/alertas/" + idA + "/historico");
                    if (hist.trim().equals("[]") || hist.trim().isBlank()) {
                        linha("\n  Nenhuma acao registrada para este alerta.");
                        linha("  Use a opcao 6 para resolver e registrar uma acao.");
                    } else {
                        exibir(hist);
                    }
                }
                case "6" -> {
                    System.out.print("> ID do alerta a resolver: ");
                    String idA = scanner.nextLine().trim();
                    // Confirma que o alerta existe e esta pendente
                    String alerta = get("/alertas/" + idA);
                    if (alerta.contains("\"erro\"")) {
                        exibir(alerta);
                        break;
                    }
                    boolean jaResolvido = alerta.contains("\"flResolvido\": \"S\"") || alerta.contains("\"flResolvido\":\"S\"");
                    if (jaResolvido) {
                        linha("\n  Este alerta ja foi resolvido anteriormente.");
                        break;
                    }
                    System.out.print("> ID do usuario que esta resolvendo (Enter para omitir): ");
    ...
    // Extrai valor de um campo simples do JSON retornado pela API
    static String extrairCampo(String json, String campo) {
        String chaveStr = "\"" + campo + "\":\"";
        int i = json.indexOf(chaveStr);
        if (i >= 0) {
            i += chaveStr.length();
            int f = json.indexOf("\"", i);
            return f >= 0 ? json.substring(i, f) : "";
        }
        // Campo numerico (latitude, longitude, etc.)
        String chaveNum = "\"" + campo + "\":";
        i = json.indexOf(chaveNum);
        if (i >= 0) {
            i += chaveNum.length();
            int f = json.indexOf(",", i);
            int f2 = json.indexOf("}", i);
            int fim = (f >= 0 && f < f2) ? f : f2;
            return fim >= 0 ? json.substring(i, fim).trim() : "";
        }
        return "";
    }

    // Exibe valor atual em colchetes — Enter mantem, qualquer texto substitui
    static String lerOuManter(String atual, String prompt) {
        System.out.printf("> %s [%s]: ", prompt, atual);
        String novo = scanner.nextLine().trim();
        return novo.isEmpty() ? atual : novo;
    }

    // Igual lerOuManter mas converte para maiusculo (perfil, estado)
    static String lerOuManterUpper(String atual, String prompt) {
        System.out.printf("> %s [%s]: ", prompt, atual);
        String novo = scanner.nextLine().trim();
        return novo.isEmpty() ? atual : novo.toUpperCase();
    }
