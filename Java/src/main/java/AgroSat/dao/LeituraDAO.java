...
public class LeituraDAO {
    ...
    // Deleta em cascata: historico → alertas → previsoes → leitura
    public boolean deletar(Long id) {
        ...
        try {
            ...
            // 1. Historico de alertas vinculados as previsoes desta leitura
            ps = conn.prepareStatement(
                    "DELETE FROM TB_HISTORICO_ALERTA WHERE id_alerta IN " +
                            "(SELECT id_alerta FROM TB_ALERTA WHERE id_previsao IN " +
                            "(SELECT id_previsao FROM TB_PREVISAO WHERE id_leitura = ?))");
            ps.setLong(1, id);
            ps.executeUpdate();
            ps.close();

            // 2. Alertas vinculados as previsoes desta leitura
            ps = conn.prepareStatement(
                    "DELETE FROM TB_ALERTA WHERE id_previsao IN " +
                            "(SELECT id_previsao FROM TB_PREVISAO WHERE id_leitura = ?)");
            ps.setLong(1, id);
            ps.executeUpdate();
            ps.close();

            // 3. Previsoes desta leitura
            ps = conn.prepareStatement(
                    "DELETE FROM TB_PREVISAO WHERE id_leitura = ?");
            ps.setLong(1, id);
            ps.executeUpdate();
            ps.close();
            ...
            // 4. A leitura em si
            ps = conn.prepareStatement(
                    "DELETE FROM TB_LEITURA_SATELITAL WHERE id_leitura = ?");
            ps.setLong(1, id);
